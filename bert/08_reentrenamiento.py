# 08_reentrenamiento.py
# Aquí una vez hemos preparado los JSON con los datos de nuestra categoría, volvemos a entrenar al modelo que ya teníamos con estos nuevos datos.
# Una vez que terminemos podemos volver a lanzar el 06_metricas.py para ver que tal ha ido.
        ...
        if file_name.endswith('.json'):
            ...
            with open(file_path, 'r', encoding='utf-8') as file:
                ...
                for item in data:
                    ...
                    # Asegúrate de que los tokens estén dentro del rango del vocabulario
...
nuevos_datos_dir = 'C:/Users/34670/Desktop/python/Hack a boss/proyecto_decide/bert/nuevos_datos/'
...
nuevos_encodings, nuevos_labels = load_preprocessed_data(nuevos_datos_dir, tokenizer)

# Combinar los datos existentes con los nuevos datos
combined_encodings = {key: torch.cat((val, nuevos_encodings[key])) for key, val in encodings.items()}
combined_labels = torch.cat((labels, nuevos_labels))

# Verificar que las longitudes coincidan
assert len(combined_encodings['input_ids']) == len(combined_labels), "Inconsistencia en el número de muestras entre encodings y labels"

# Dividir en conjuntos de entrenamiento y evaluación
indices = list(range(len(combined_labels)))
train_indices, val_indices = train_test_split(indices, test_size=0.2, random_state=42)

train_encodings = {key: val[train_indices] for key, val in combined_encodings.items()}
val_encodings = {key: val[val_indices] for key, val in combined_encodings.items()}
...
# Tengo que decir que aquí me he quedado sin tiempo para seguir trabajando en el proyecto al vecener el plazo